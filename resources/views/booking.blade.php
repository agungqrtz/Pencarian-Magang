...
<div class="relative w-full max-w-[420px] min-h-screen bg-gray-100">
    ...
    <div class="p-4 space-y-4 pb-36">
        ...
        <div class="bg-white rounded-xl p-4 shadow">
            ...
            <div id="slot-container" class="space-y-3 text-sm text-center text-gray-400">
    ...
    <div class="fixed bottom-0 left-1/2 -translate-x-1/2
        ...
        <a href="{{ route('profile') }}" class="flex flex-col items-center text-gray-500">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-8 h-8">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
            </svg>
        </a>

        <a href="{{ route('pengaturan') }}" class="flex flex-col items-center text-gray-500">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-8 h-8">
...
<script>
$(document).on('click', '.tanggal:not([disabled])', function () {
    $('.tanggal').removeClass('bg-blue-500 text-white');
    $(this).addClass('bg-blue-500 text-white');

    let tanggal = $(this).data('date');
    let slug = "{{ $dosen->slug }}";

    $('#slot-container').html('Loading...');

    $.get(`/dosen/${slug}/slots`, { tanggal }, function (res) {
        let html = '';

        if (res.length === 0) {
            html = '<p class="text-gray-400">Tidak ada slot</p>';
        }
                ...
                <div class="flex justify-between items-center border-b pb-2">
                    ...
                    ${
                        slot.status === 'booked'
                            ...
                            : `<button class="bg-blue-500 text-white text-xs px-3 py-1 rounded btn-book"
...
$(document).on('click', '.btn-book', function () {
    ...
    $.post(`/dosen/${slug}/booking`, {
        _token: $('meta[name="csrf-token"]').attr('content'),
        tanggal: $(this).data('tanggal'),
        jam_mulai: $(this).data('mulai'),
        jam_selesai: $(this).data('selesai')
    }, function () {
        alert('Slot berhasil dibooking');
        location.reload();
    });
});
</script>
