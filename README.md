a. Nama Aplikasi = Pencarian Magang b. Deskripsi Aplikasi = Pencarian Magang adalah proses pencrian magang yang dilakukan oleh mahasiswa yang direkomendasikan magang sesuai jurusan sekaligus pencarian dosen pembimbing dan update akitvitas magang yang sudah di lakukan c. 
| Kelas | NIM              | Nama                       | Username GitHub                              |
|-------|------------------|----------------------------|-----------------------------------------------|
| RK-C  | 202210370311101  | Mochamad Agung Setiawan    | [agungqrtz](https://github.com/agungqrtz)     |
| RK-C  | 202210370311139  | Prima Septian Yudantra     | [primaseptiiian](https://github.com/primaseptiiian) |
| RK-C  | 202210370311214  | Almas'Udi                  | [Almasudii](https://github.com/Almasudii)     |

d. Link Figma = https://www.figma.com/design/tuuyUtUAXaVGmyHkSSnYTO/Rekayasa-intraksi?node-id=0-1&t=Elld27EqoLX50OGZ-1 e. Worksheet = f. Low Fidelity Prototype = g. Tabel pembagian tugas (backlog) tiap anggota secara bergantian dengan level
| #  | Fitur / Task               | PIC                         | Level   | Keterangan Singkat                                  |
|----|----------------------------|-----------------------------|---------|-----------------------------------------------------|
| 1  | Beranda                    | Mochamad Agung Setiawan     | Medium  | Tampilan utama + navigasi menu                      |
| 2  | Booking Bimbingan          | Prima Septian Yudantra      | Hard    | Pemilihan jadwal + logika backend                   |
| 3  | Login                      | Almas'Udi                   | Medium  | Autentikasi + validasi                              |
| 4  | Profil                     | Mochamad Agung Setiawan     | Medium  | Lihat & edit profil dasar                           |
| 5  | Daftar Dosen Pembimbing    | Prima Septian Yudantra      | Medium  | List + filter + pilih pembimbing                    |
| 6  | Register                   | Almas'Udi                   | Medium  | Pendaftaran akun mahasiswa/dosen                    |
| 7  | Detail Lowongan Magang     | Mochamad Agung Setiawan     | Hard    | Detail info magang + UI + integrasi backend         |
| 8  | Ubah Kata Sandi            | Prima Septian Yudantra      | Easy    | Validasi password + update data                     |
| 9  | Edit Profile (advanced)    | Almas'Udi                   | Hard    | Update data + upload foto + validasi                |
| 10 | Pengaturan                 | Mochamad Agung Setiawan     | Easy    | Menu setting + preferensi akun                      |
| 11 | Dashboard Magang           | Almas'Udi                   | Hard    | Status magang + progres + tampilan aktivitas        |
| 12 | Notifikasi                 | Prima Septian Yudantra      | Medium  | Sistem notifikasi + list notifikasi                 |

